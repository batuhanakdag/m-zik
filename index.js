...
client.on('message', async msg => {
    ...
    .setDescription('❎ | Lütfen Seli Bir Kanala Giriş Yapınız!'));
     ...
     .setColor('RANDOM')
     .setTitle('❎ | Şu An Zaten Şarkı Çalmıyorum!'));                                              
		msg.channel.send(`:police_officer:  **${serverQueue.songs[0].title}** Adlı Şarkı Fbi Tarafınfan Kapatıldı`) .then(a =>{a.delete(2000)}) ;
		msg.delete(5000)
    serverQueue.songs = [];
...
function play(guild, song) {
  ...
  .setColor('RANDOM')) .then(a =>{a.delete(2000)}) ;
}
